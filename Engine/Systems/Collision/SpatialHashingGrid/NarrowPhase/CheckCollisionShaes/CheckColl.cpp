...
Vector<VecPos> CheckColl::Point::GetClosestPointsOnSidesOfSqToCircle(
    ...
    for (std::size_t point = 0; point < rectangle->getPointCount(); ++point) {
        const sf::Vector2f StartLinePoint = rectangle->getTransform().transformPoint(
...
Vector<VecPos> CheckColl::Extract::ExtractPointFromShape(const sf::Shape* shape) {
    ...
    for (std::size_t point = 0; point < shape->getPointCount(); ++point) {
        DataExtracted.Append( { CheckColl::Point::GetPoint( shape->getPoint( point ), shape ) } );
    }
...
VecPos CheckColl::Point::GetClosestPointOnSqToCircle(
    const Vector<VecPos>& points, const Circle* target_circle) {
    sf::Vector2f closestPoint = { points[0].get_x(), points[0].get_y() };

    float closestDistance = (closestPoint - target_circle->getPosition()).lengthSquared();

    for (std::size_t point = 1; point < points.Size(); ++point) {
        const float currentDistance = (sf::Vector2f { points[point].get_x(), points[point].get_y() }
                - target_circle->getPosition())
            .lengthSquared();
...
bool CheckColl::Point::PointIsInShape(const VecPos& target_point, const sf::Shape* target_shape) {
    ...
    for (std::size_t current_point_index = 0; current_point_index < point_count;
         ++current_point_index) {
        ...
        const float intersection_x = PreviousShapePoint.get_x()
            + (CurrentShapePoint.get_x() - PreviousShapePoint.get_x())
            * (target_point.get_y() - PreviousShapePoint.get_y())
            / (CurrentShapePoint.get_y()
                - PreviousShapePoint.get_y());
...
VecPos CheckColl::Point::GetPoint(const sf::Vector2f& point, const sf::Shape* shape) {
    const sf::Transform transform = shape->getTransform();
    const sf::Vector2f WorldPoint = transform.transformPoint( point );
    return VecPos { WorldPoint.x, WorldPoint.y };
}

VecPos CheckColl::SAT::Normalize(const VecPos& point) {
    VecPos NormalizedPoint = { point.get_x(), point.get_y() };
    const float length = std::sqrt(
        NormalizedPoint.get_x() * NormalizedPoint.get_x()
        + NormalizedPoint.get_y() * NormalizedPoint.get_y() );
    if (length == 0.0f) {
        return { 0.0f, 0.0f };
    }
...
VecPos CheckColl::SAT::GetNormalLine(const VecPos& FirstPoint, const VecPos& SecondPoint) {
    ...
    edge = { -edge.get_y(), edge.get_x() };
    return Normalize( edge );
}

Vector<float> CheckColl::SAT::GetMinAndMaxValue(const Vector<float>& values) {
...
Vector<float> CheckColl::SAT::GetPointsOfShapeOnNormalLine(
    const sf::Shape* shape, const VecPos& NormalLine) {
    Vector<float> result;
    const std::size_t ShapePointCount = shape->getPointCount();
    for (std::size_t point = 0; point < ShapePointCount; ++point) {
        result.Append(
            CheckColl::SAT::DotProduct(
                CheckColl::Point::GetPoint( shape->getPoint( point ), shape ), NormalLine ) );
    }
    return result;
}
...
Set<sf::Shape*> CheckColl::CircleCircle(sf::Shape* target_shape /* circle */) const {
    const Set<sf::Shape*> CirclesAsideCircle
        = CheckColl::Filter( CGW.Query( target_shape ), Shape_Type::Circle );
    ...
    for (auto& SHAPE : CirclesAsideCircle) {
        ...
        const float distance_x = std::abs(
            dynamic_cast<Circle*>(target_shape)->getPosition().x
            - dynamic_cast<Circle*>(SHAPE)->getPosition().x );
        const float distance_y = std::abs(
            dynamic_cast<Circle*>(target_shape)->getPosition().y
            - dynamic_cast<Circle*>(SHAPE)->getPosition().y );
...
Set<sf::Shape*> CheckColl::CircleRectangle(sf::Shape* target_shape /* circle */) const {
    const Set<sf::Shape*> RectanglesAsideCircle
        = CheckColl::Filter( CGW.Query( target_shape ), Shape_Type::Rectangle );
    ...
    for (sf::Shape* SHAPE : RectanglesAsideCircle) {
        ...
        VecPos ClosestPoint = CheckColl::Point::GetClosestPointOnSqToCircle(
            CheckColl::Point::GetClosestPointsOnSidesOfSqToCircle(
                rect, dynamic_cast<Circle*>(target_shape) ),
            dynamic_cast<Circle*>(target_shape) );
...
Set<sf::Shape*> CheckColl::CirclePolygon(sf::Shape* target_shape) const {
    const Set<sf::Shape*> PolygonsAsideCircle
    ...
    for (sf::Shape* SHAPE : PolygonsAsideCircle) {
        ...
        VecPos ClosestPoint = CheckColl::Point::GetClosestPointOnSqToCircle(
            CheckColl::Point::GetClosestPointsOnSidesOfSqToCircle(
                SHAPE, dynamic_cast<Circle*>(target_shape) ),
            dynamic_cast<Circle*>(target_shape) );
        const VecPos& Differance
            = {
                ClosestPoint.get_x() - target_shape->getPosition().x,
